...
int hFile::Create(const char *f_path, fType f_type, void *startup)
{
...
void hFile::Load(Schema &f_schema, const char *loadpath) {
	Record temp_rec;
...
int hFile::Open(const char *f_path) {
	//todo:check again
	 heapfile->Open(1, f_path);
	return 1;
}

void hFile::MoveFirst() {
	cur_page = 0;
}
...
*/
int hFile::GetNext(Record &fetchme) {
...
int hFile::GetNext(Record &fetchme, CNF &cnf, Record &literal) {
	int ret;
...
int hFile::GetPage(Page *putItHere, off_t whichPage) {
